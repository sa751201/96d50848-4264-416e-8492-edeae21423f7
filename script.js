...
async function init() {
    // Load the Maps JS API dynamically using the NEW loader
    const apiKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY;
    
    (g=>{var h,a,k,p="The Google Maps JavaScript API",c="google",l="importLibrary",q="__ib__",m=document,b=window;b=b[c]||(b[c]={});var d=b.maps||(b.maps={}),r=new Set,e=new URLSearchParams,u=()=>h||(h=new Promise(async(f,n)=>{await (a=m.createElement("script"));e.set("libraries",[...r]+"");for(k in g)e.set(k.replace(/[A-Z]/g,t=>"_"+t[0].toLowerCase()),g[k]);e.set("callback",c+".maps."+q);a.src=`https://maps.${c}apis.com/maps/api/js?`+e;d[q]=f;a.onerror=()=>h=n(Error(p+" could not load."));a.nonce=m.querySelector("script[nonce]")?.nonce||"";m.head.append(a)}));d[l]?console.warn(p+" only loads once. Re-writing importLibrary."):d[l]=(f,...n)=>r.add(f)&&u().then(()=>d[l](f,...n))})({
        key: apiKey,
        v: "weekly",
    });
    ...
    try {
        ...
        statusMsg.innerText = "Google Maps API 加載失敗，請檢查金鑰限制。";
    }
}
...
pickBtn.addEventListener('click', async () => {
    if (!Place) {
        statusMsg.innerText = "API 尚未準備就緒，請稍候...";
        return;
    }
